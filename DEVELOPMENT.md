...
最后核对：2026-08-18（Editor 面板 VM 编排分支补测 12 用例（9bcb7aa）+ 一轮 CI 红灯修复（da84fd7）：EditorPanelVMTests 与 EditorViewModelTests 互补覆盖缝隙——调色（中性 reset no-op/滤镜缩略图 photoGeneration 缓存与失效/resetSharpness 保留 renderedSharpness 基准）、裁剪（updateCropRect clamp）、文字（showEditPanel 退出文本工具时显示/updateActiveText photo 层 no-op/deleteActiveLayer 保护底图）、文档（点选顶层优先底图不可选/贴纸视觉钳制/层级钳制/旋转累计/addPassive 不抢占）、抠图（GatedVisionService 挂起验证 processing 重入拒绝）；修复教训：shouldShowTextLayerEditPanel = text 层激活且 tool != .text（断言方向勿凭直觉）；其间处置 git push 网络故障——全局 URL 级代理键 http.https://github.com.proxy 指向未运行的本机 7892 端口，git -c 单次置空后直连仍被 reset，改走 ssh.github.com:443 临时 URL 推送（SSH key 已注册，不动 remote 配置）；CI 全绿 run 32075135798：Kit 1116（Linux）+ App 1171（=1159+12，模拟器）+ UI 6 = 2293 用例 0 失败，覆盖率门禁 PASS（App 加权 line/function 均 24.4% 持平——面板 VM 加权贡献小但 5 文件脱离零测试；最差 5 文件仍全为 0% View 渲染层））；此前同日：零覆盖纯逻辑文件补测 49 用例（2ba17b3）+ 两轮 CI 红灯修复（d7aeec9/8b2002f）：新建 TimelineArchiveLogicTests 18（档案统计 photoCount 透传/作品过滤/重要日子/相伴天数领养日优先钳 0、置顶记忆 pinned 优先与用户文本回退、相处章节按年分组末章标记、年度回看每月第一条、删除边界）、PhotoMetadataLogicTests 11（EXIF 双格式/引号空白剥离/ISO8601 降级/无效 nil/兜底拍摄时间三级回退）、RedPacketQualityInputBuilderTests 12（清晰度原图亮度抠图优先/面积公式钳 1/安全区归零/WCAG 对比度/蒙版四象限）、RedPacketDraftStoreTests 8（往返/覆盖/listAll 降序/损坏跳过/抠图 PNG/delete 联动幂等）；修复教训：双引号 here-string 展开闭包隐式参数致 map 简写落盘丢参数引用（改 key path）、Date() 亚秒经 ISO8601 往返毫秒截断致不等（构造整秒）；CI 全绿 run 31995526845：Kit 1116（Linux）+ App 1159（=1110+49，模拟器）+ UI 6 = 2281 用例 0 失败，覆盖率门禁 PASS（App 加权 line/function 均 24.4%，24.1%->24.4%，基线 18/18/0；最差 5 文件仍全为 0% View 渲染层））；此前同日：AiInference 周边与 Import/Quality 错误分支补测 31 用例（fb6076e）+ 两轮 CI 红灯修复（4ff8729/6083058）：新建 ClipInferenceServiceTests 17（CLIP 推理：资源目录布局/加载失败降级/预处理构造与批次切分/嵌入输出形状）、PetTextEmbeddingsTests 4（bundle 资源布局/文件缺失/维度不匹配）、ImportServiceTests 24->31（重入互斥/Pro 放行/取消/目录创建失败环境级终止/内存去重降级 + DB unique 双层防御兜底/flush 失败回滚计数）、QualityScorerTests 8->11（wrapper 注入仓储三失败分支）；FileStorage 补 failCreateDirectory 开关；修复教训：CGImage bitmapInfo 需 CGBitmapInfo(rawValue:) 桥接（Windows 无 iOS SDK 只能 CI 暴露，对照既有测试先例）、辅助返回的 ModelContainer 解构到 _ 会立即释放致 SIGTRAP（须命名绑定保活）；CI 全绿 run 31978432202：Kit 1116（Linux）+ App 1110（=1079+31，模拟器）+ UI 6 = 2232 用例 0 失败，覆盖率门禁 PASS（App 加权 line/function 均 24.1%，23.8%->24.1%，基线 18/18/0；最差 5 文件仍全为 0% View 渲染层））；此前同日：Service 层与 Gallery 分支补测 17 用例（46c0eef）：ScanServiceTests 19→24——扫描暂停/恢复/取消状态机：阶段 1 进度回调内同步 pauseScan 存断点（断点=下一资产）/resume 跳过断点前与断点本身（updateResumePoint 语义）/阶段 2 批次间暂停/resetScanState 全新全量/取消清断点，onProgress 在 streamPhotos consumer 内同步执行故无需自制暂停 mock；NotifyServiceTests 14→18——备份提醒三态（从未备份/恰 60 天过期/59 天未达阈值）+ 次日 09:00 日期组件断言 + 不满足时先幂等清理旧通知；GalleryViewModelTests 13→21——60 张分页边界（61 首页 60+hasMore/恰好 60 需空页确认结束/无更多 no-op）+ 宠物与收藏叠加筛选不动原始分页数组 + setFavorite 仓储失败保留状态（InMemoryPhotoRepository 新增 setFavoriteError 失败注入开关，final class 不能子类化故用属性开关）；CI 全绿 run 31972006447：Kit 1116（Linux）+ App 1079（=1062+17，模拟器）+ UI 6 = 2201 用例 0 失败，覆盖率门禁 PASS（App 加权 line/function 均 23.8%，23.7%→23.8%，基线 18/18/0；最差 5 文件仍全为 0% View 渲染层）；此前同日：5 个 0% ViewModel 补测 75 用例（deb33e5）+ 三轮 CI 红灯修复（1cca26e/985a7a5/bb5abae）：MiLensTests 新增 PetProfileViewModelTests 10（列表/建档校验含免费上限 1 只与 Pro 放行/07-03 彩蛋/删除）、BackupViewModelTests 11（预估/导出写 lastBackupDate/恢复统计/失败态/幂等互斥，Mock 去 @MainActor 改 @unchecked Sendable 满足协议 nonisolated witness）、TimelineViewModelTests 12（月度组装/免费档一年锁定/全史预览/回忆写入）、HomeViewModelTests 20（问候分段/load 装配与失败兜底/hero 三级选片/新照片提醒 4 分支/备份横幅阈值/即将纪念日含 memorial 与缩略图，escaping 闭包 self. 前缀 + setUp assumeIsolated）、PetEditViewModelTests 22（加载保存/特征注册成功失败取消）；修复教训：XCTest 含 await 方法必须 async 签名、removePersistentDomain(forName:) 标签、VM 内存列表型校验须先 loadPets 再断言；CI 全绿 run 31970133746：Kit 1116（Linux）+ App 1062（=987+75，模拟器）+ UI 6 = 2184 用例全绿，覆盖率门禁 PASS（App 加权 line/function 均 23.7%，22.6%→23.7%，基线 18/18/0；最差 5 文件仍全为 0% View 渲染层）；此前同日：BeadViewModel 单测补齐（6159ff3）：MiLensTests/BeadViewModelTests 22 用例——load 路由/generate 三层守卫（重入/每日配额/缺源图）/成功失败取消路径/CLIP 语义按源图缓存/export 与 prepareShare/PDF/toast 自动清除，依赖全协议注入（InMemory+Mock+QuotaSpy），BeadViewModel（423 xccov 行）脱离最差 5 文件清单；6159ff3 遗留的 2 条 tearDown nonisolated 访问警告由 171122b 改 addTeardownBlock 消除（闭包继承 @MainActor 隔离）；CI 全绿 run 31964693373：Kit 1116（Linux）+ App 987（=965+22，模拟器）+ UI 6 = 2109 用例全绿，覆盖率门禁通过（App line/function 加权均 22.6%，基线 18/18/0；最差 5 文件均换为 0% View 文件，下轮补测目标见 PLAN.md）；此前同日：质量复评 N1-N8 修复收口：N1 两处 Onboarding stageRow 编译错误（CI 红灯根因）；N7 Swift 6 严格并发警告源码层清理（App/Widget/Kit 17 文件）+ PetEditView 头像按钮三元分支语义修正；N6 UI 测试 11 处中文文案断言迁移 accessibilityIdentifier（identifier = loc key 约定，6 个视图文件同步标注）；N8 token lint 143 处 INFO 复评维持不判罚（docs/UI-Rework计划.md 批次 A）；N2 main 分支保护落地（见 §2.2）；收口 CI 全绿 run 31961932575（N1-N8 修复 2cdb662 + 本地化回退 b3b0a79）：Kit 1116（Linux）+ App 965（模拟器）+ UI 6 = 2087 用例全绿，覆盖率门禁通过（App line/function 加权均 21.9%，基线 18/18/0）；其间排查并回退 1c623e9 误入主 catalog 的 117 条 en 初译——en 模拟器 preferred localization 变为 en 后其余 key 缺译返回 key 本身，App Test exit 65，en 初译仍以 xlsx 为唯一事实来源（红线见 docs/Localization-Plan.md §5.4）；此前 2026-08-16：P0–P5 实现完成；CI 全绿 run 31905505907：Kit 1113（Linux）+ App 943（模拟器）+ UI 6 = 2062 用例全绿，覆盖率门禁基线回调 18/18/0（App 加权 line/function 21.3%）；真机与性能验收待做，清单见 docs/P2-待办清单.md）

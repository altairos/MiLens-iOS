...
- 2026-08-17：**BeadViewModel 单测补齐（App 覆盖率 21.9% → 22.6%）**——上轮门禁最差 5 文件中唯一纯 ViewModel `BeadViewModel`（423 xccov 行，0%）补齐 22 用例（`MiLensTests/BeadViewModelTests.swift`，6159ff3）：load 路由 / generate 三层守卫（重入 / 每日配额 / 缺源图）/ 成功-失败-取消路径 / CLIP 语义按源图缓存 / export 与 prepareShare / PDF / toast 自动清除；依赖全协议注入（InMemory 仓储 + Mock 服务 + QuotaSpy）；临时文件清理用 `addTeardownBlock`（171122b，闭包继承 @MainActor 隔离，消除 nonisolated tearDown 访问 actor 属性的 2 条 Swift 6 毕竟错误警告）。**CI 全绿 [run 31964693373](https://github.com/altairos/MiLens-iOS/actions/runs/31964693373)**：Kit 1116（Linux）+ App 987（=965+22，模拟器）+ UI 6 = 2109 用例 0 失败；gate PASS（App 加权 line/function 均 22.6%，基线 18/18/0）。

  **下轮补测目标（方法沿用本轮，非文件清单）**：本轮后门禁最差 5 文件全部为 0% View 渲染层（TimelineView 1152 / BeadPatternResultView 1145 / PhotoViewView 859 / RedPacketUploadGuideView 612 / TimelineMemoryCards 527 xccov 行，含宏展开）——P1-4 已将其决策逻辑下沉纯 struct 并有 45 用例守护，剩余未覆盖的是 SwiftUI body 本身，单测性价比低，由 UI 冒烟（6 条）+ 真机走查覆盖即可；继续提升覆盖率应改为从 xcresult artifact 的 xccov 报告定位其余 0%/低覆盖的 **Service/ViewModel**（如 GalleryViewModel 未覆盖分支、HomeViewModel、BackupViewModel 等），按「协议注入 + 行为路径断言」模式逐个补测，性价比高于堆 View 测试。

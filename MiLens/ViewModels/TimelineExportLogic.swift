...
enum TimelineExportLogic {
    ...
    static func buildExportData(
        ...
        let dateRangeText = buildDateRange(from: months, calendar: calendar)
    ...
    /// 从月份列表提取日期范围文案。
    /// 最早月份 → 最晚月份，格式「YYYY-MM — YYYY-MM」。
    private static func buildDateRange(
        from months: [TimelineMonth], calendar: Calendar
    ) -> String {
        guard let first = months.first, let last = months.last else { return "" }
        if first.yearMonth == last.yearMonth {
            return first.yearMonth.replacingOccurrences(of: "-", with: "-")
        }
        return "\(first.yearMonth) — \(last.yearMonth)"
    }
}

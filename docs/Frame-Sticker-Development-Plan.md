...
- [ ] 修复快照字段、相框中心点、稳定合成顺序和画布重映射。
- [ ] 完成共享素材解析器，使 preview/export 对 `ratioSet` 与 `ninePatch` 一致。
- [ ] 补 catalog 分组 ID、本地化映射和素材错误诊断。
- [ ] MiLensKit 新增纯逻辑测试，App 层新增渲染单测。

### M1：可用交互与首批素材

- [ ] 实现共用素材单元、分类轨、空态和 Pro 锁定态。
- [ ] 实现相框单选替换/移除，以及贴纸添加/选择/手势/删除。
- [ ] 导入并校验 6 个相框、6 个贴纸；补齐简中源文案与其他已支持语言 key。
- [ ] 接入撤销/重做、保存、返回未保存确认和付费墙。

...
- [ ] 素材交付（进行中）：贴纸 12/12 已入库（首批 6 + 第二批候选 5 + 规划外 Tandem Paws；玩具球未做）；相框 1/6 已入库——`frame_spring_botanical`（paper 组，`ratio_set` 仅 `3x4`，2026-08-24 用户批准单比例交付，偏差见[开发计划](docs/Frame-Sticker-Development-Plan.md) §6.2）。catalog 非空 → decorate 组贴纸/相框入口已按门禁自动出现。剩余：首批规划相框 5 个。12 条 `decoration.sticker.*` a11y key 已补录（7 语言，2026-08-24；顺带补齐 `decoration.sticker.limit %lld` 缺失的 ko）。
...
- 2026-08-17：**BeadViewModel 单测补齐（App 覆盖率 21.9% → 22.6%）**——上轮门禁最差 5 文件中唯一纯 ViewModel `BeadViewModel`（423 xccov 行，0%）补齐 22 用例（`MiLensTests/BeadViewModelTests.swift`，6159ff3）：load 路由 / generate 三层守卫（重入 / 每日配额 / 缺源图）/ 成功-失败-取消路径 / CLIP 语义按源图缓存 / export 与 prepareShare / PDF / toast 自动清除；依赖全协议注入（InMemory 仓储 + Mock 服务 + QuotaSpy）；临时文件清理用 `addTeardownBlock`（171122b，闭包继承 @MainActor 隔离，消除 nonisolated tearDown 访问 actor 属性的 2 条 Swift 6 编译错误警告）。**CI 全绿 [run 31964693373](https://github.com/altairos/MiLens-iOS/actions/runs/31964693373)**：Kit 1116（Linux）+ App 987（=965+22，模拟器）+ UI 6 = 2109 用例 0 失败；gate PASS（App 加权 line/function 均 22.6%，基线 18/18/0）。

- 2026-08-17：**5 个 0% ViewModel 补测 75 用例（App 覆盖率 22.6% → 23.7%）**——上轮点名的 HomeViewModel/BackupViewModel 加上 PetProfileViewModel/TimelineViewModel/PetEditViewModel 共 5 个零测试 VM 一次性补齐（deb33e5）：PetProfileViewModelTests 10（列表加载/建档校验含免费上限 1 只与 Pro 放行/07-03 同日生日彩蛋/删除失败兜底）、BackupViewModelTests 11（预估/导出写 lastBackupDate/恢复统计/失败态/幂等互斥，Mock 去 @MainActor 改 @unchecked Sendable 满足协议 nonisolated witness，进度回调 await）、TimelineViewModelTests 12（月度组装/免费档一年锁定与预览解锁/回忆写入成功失败）、HomeViewModelTests 20（问候分段/load 装配与失败兜底/hero 三级选片/新照片提醒 4 分支/备份横幅 20 张与 30 天阈值/即将纪念日含 memorial 与缩略图，setUp assumeIsolated 备份 UserDefaults）、PetEditViewModelTests 22（加载保存/特征注册成功-失败-取消含进度与 CLIP 缓存）。**三轮 CI 红灯修复**：①1cca26e——BackupViewModelTests 15 编译错（removePersistentDomain(forName:) 标签 + 9 方法补 async + Mock 隔离）与 HomeViewModelTests 隐患（escaping 闭包 24 处 self. 前缀 + nonisolated setUp 访问 @MainActor static）；②985a7a5——PetEditViewModelTests L389 漏标 try；③bb5abae——免费上限用例漏调 loadPets（addPet 校验用 VM 内存列表，构造后列表为空未拦截，断言前先 load）。**CI 全绿 [run 31970133746](https://github.com/altairos/MiLens-iOS/actions/runs/31970133746)**：Kit 1116（Linux）+ App 1062（=987+75，模拟器）+ UI 6 = **2184 用例 0 失败**；gate PASS（App 加权 line/function 均 23.7%，基线 18/18/0；最差 5 文件仍全为 0% View 渲染层，与上轮一致）。

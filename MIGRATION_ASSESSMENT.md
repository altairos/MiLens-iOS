...
| 完整图片编辑器（裁剪/调色/抠图/相框/文字/贴纸） | **✅ 已纳入 V1.0** | 产品决策更新（[ADR-0008](docs/adr/0008-v1-scope-decision.md)）：完整迁移源端 `editor/` ~30 文件，归 P4 |
| AI 写真/回忆视频 | **iOS 新增，需新方案** | 源端无对应，需额外设计（可能依赖云服务，待产品决策） |
| 质量评分/重复分组 | **✅ 已纳入 V1.0** | 产品决策更新（[ADR-0008](docs/adr/0008-v1-scope-decision.md)）：扫描流程扩展（QualityAnalyzer + embedding 去重），归 P2 |

> ✅ 范围已定案（[ADR-0008](docs/adr/0008-v1-scope-decision.md)，P0 收口）。上表中标注「✅ 已纳入 V1.0」的项为产品决策更新结果，不再暂缓。

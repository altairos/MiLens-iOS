...
final class IOSFileStorageTests: XCTestCase {
    ...
    // MARK: - 备份排除（导入副本排除备份；编辑成品允许备份）

    /// 写入 Documents/MiPhotos 下的导入副本必须设置 isExcludedFromBackup
    /// （可重建——原图在系统相册可重新导入；照片不离开设备的承诺）。
    /// 使用测试沙盒的真实 Documents 目录（唯一子目录，结束后清理）。
    func testDocumentsWriteExcludedFromBackup() async throws {
        ...
        XCTAssertEqual(values.isExcludedFromBackup, true,
                       "MiPhotos 下的导入副本必须排除备份（可重建，照片不离开设备）")
    }

    /// 写入 Documents/MiPhotos/Edits 下的编辑成品**不**排除备份——
    /// 编辑后版本不在系统相册、不可重建，设备恢复后必须能随备份还原（评审阻塞项）。
    func testEditsWriteKeepsBackupEligible() async throws {
        let fs = IOSFileStorage()
        let dir = URL.documentsDirectory
            .appendingPathComponent(ScanConfig.sandboxDirName)
            .appendingPathComponent(ScanConfig.editsDirName)
            .appendingPathComponent("IOSFileStorageBackupTest-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
        let fileURL = dir.appendingPathComponent("MiLens_Edit_1.jpg")

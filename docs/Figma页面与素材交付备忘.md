...
- V1 的一级导航只有四个 Tab：**首页、宠物、创作、我的**。相册、照片详情、编辑器、时间线和创作流程是二级页面，不新增 Tab。
- V1 创作首页只展示目前能真实完成的两项：**拼豆图纸、宠物卡片**。AI 写真、回忆视频、商城、社区、账号和手表不应进入 V1 Figma 主流程。
...
- 当前仓库的 `Assets.xcassets` 主要是颜色 token 和 App Icon，已有两套字体（霞鹜文楷、Fraunces）；**品牌插画、宠物示例图、装饰素材、相框、贴纸、相簿皮肤目前都没有作为正式资源交付**。
...
7. `06 · Pets & Timeline`：宠物列表、档案、编辑档案、时间线、时间线导出。
8. `07 · Create`：创作首页、拼豆流程、宠物卡片流程。
...
| P-01–04 | 宠物列表、档案、编辑档案、添加宠物 | 已有 | P0 |
...
| PC-01–03 | 宠物卡片选图、预览、分享/导出 | 已有单模板/模板骨架，需补多模板和状态 | P0/P1 |
...
| O-03 扫描 | 扫描中进度；已检查/可能含宠物/等待确认；暂停/取消；扫描失败；无候选；跳过扫描；后台恢复提示 |
...
| Frame | 必须制作的画面 |
|---|---|
| T-01 四 Tab | 首页/宠物/创作/我的选中与未选中；导航栏标题；iPhone；iPad Sidebar/Detail 变体；深色模式 |
| H-01 有今日照片 | 问候/日期；一张主回忆 Hero；照片外的日期和宠物归属；往年今日或纪念日；唯一主动作“为它创作” |
| H-02 无今日照片 | 回退到最近照片；真实拍摄日期；无照片时进入相册的动作 |
| H-03 临近纪念日 | 纪念内容提升；生日/领养日文案；生成纪念卡片 CTA（若该能力尚未上线，稿中标注 Future，不放进 V1 主原型） |
| H-04 多宠物 | Hero 及回忆内容包含宠物归属；进入对应档案 |
...
| Frame | 必须制作的画面 |
|---|---|
| G-01 相册内容 | 连续照片网格；照片质量标记；重复分组标记；收藏；宠物筛选；待整理筛选；工具栏扫描入口 |
| G-02 相册空态 | 还没有照片；扫描系统相册；手动选择照片；Limited Photos 提示 |
| G-03 多选 | 选中态；全选/取消；吸底操作栏：归档到宠物、收藏、删除；删除二次确认并说明只处理 App 沙盒副本 |
| G-04 扫描工具页 | 扫描中、暂停、取消、完成、失败、可恢复；“可能含宠物”而不是确定识别文案 |
| G-05 导入确认 | 候选照片预览；选择宠物/新建档案；导入中；导入完成；配额超限/Pro 付费墙 |
...
| Frame | 必须制作的画面 |
|---|---|
| P-01 宠物列表 | 有档案；头像/肖像；照片数；最近更新；进入档案；新增宠物；空态；加载/失败 |
| P-02 宠物档案 | 肖像 Hero；名字、物种、生日、领养日；照片分组；成长时间线入口；编辑入口；创作入口 |
| P-03 档案照片 | 全部/已编辑/未归属筛选；照片网格；照片详情；空筛选结果 |
| P-04 编辑档案 | 名字、物种、生日、领养日、备注/肖像；保存中；校验失败；删除档案二次确认；纪念提醒联动说明 |
| P-05 添加宠物 | Sheet/页面形式；字段为空、保存、取消、保存失败 |
...
| Frame | 必须制作的画面 |
|---|---|
| TL-01 时间线 | 按年月分组；生日/领养日/照片事件；宠物筛选；时间线为空；无事件；大字号改用 Menu |
...
| Frame | 必须制作的画面 |
|---|---|
| PH-01 大图 | 沉浸式照片；缩放/拖拽；返回；编辑；分享；照片元数据/宠物归属；加载/文件缺失 |
...
| Frame | 必须制作的画面 |
|---|---|
| C-01 创作首页 | “让照片继续有去处”叙事；拼豆图纸入口；宠物卡片入口；原图→成品示例；没有照片时去相册 |
...
| Frame | 必须制作的画面 |
|---|---|
| PC-01 选照片 | 照片选择；宠物信息预览；无照片；取消/确认 |
...
| 宠物示例图 | 猫/狗、横/竖、单宠/多宠、浅/深背景、低质量、无头像 | 用于 Home Hero、Create 示例、相册和编辑器测试；必须有可授权来源 |
| 拼豆示例 | 原图→拼豆成品对照；不同尺寸/色板；A4 结果页 | 当前入口会用真实用户照片生成示意，不等于正式营销素材 |
| 宠物卡片示例 | 默认模板至少 3 组真实示例；名字/日期长短变化 | 覆盖无领养日、长名字、多语言/大字号、深色照片 |
| 空态/错误插画 | 欢迎、无照片、无宠物、扫描失败、文件缺失、数据库恢复 | 按 Quiet Archive 方向做克制的线稿/照片型插画；不要默认使用大爪印 Emoji |
...
| 素材包 | 需要交付 | 当前事实 |
|---|---|---|
| 宠物卡片多模板 | 默认、拍立得、杂志、纪念/明信片等模板的封面、版式、字体、配色、缩略图、Pro 标记 | `PetCardTemplate` 已有模板类型/元数据骨架；需要最终视觉和逐模板验收 |
...
| 分享预览 | 拼豆、宠物卡、成长对比、时间线/年度回忆册四类预览背景、导出规格和水印位置 | `SharePreviewSheet` 已有统一入口；四类成品视觉还需统一 |
...
- “AI 认识每一只宠物”“自动建立完整档案”等确定性宣传文案：当前能力只能表达为帮助发现可能含宠物的照片。
...
- 照片必须注明来源/授权；不把真实用户照片、真实宠物数据或个人数据库放入仓库。
...
3. **完成高频内容页**：相册、照片详情、编辑器、宠物档案、时间线。
4. **完成创作闭环**：创作首页 → 拼豆 → 宠物卡片 → 分享/导出 → Pro。
5. **完成设置与异常状态**：我的、隐私、通知、外观、帮助、关于、数据库恢复。
6. **再做情感资产**：多模板宠物卡、纪念卡、成长对比、月度精选、年度回忆册。
...
10. E-01 图片编辑器默认态
11. C-01 创作首页
12. B-04 拼豆结果 / PC-02 宠物卡片二选一作为第一张作品展示页

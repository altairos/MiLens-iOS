...
| 创作 | 我能用这些照片做什么？ | 拼豆图纸、伙伴卡片；从作品选择照片 | AI 写真、回忆视频占位入口 |
| 我的 | 数据和购买由我怎样控制？ | Pro 状态、隐私、通知、外观、帮助、关于 | 假头像、账号等级、社交数据 |

图片编辑器是照片详情的上下文工具，不作为一级入口。相册是跨域内容库，从首页与伙伴档案进入，不占用新的 Tab。
...
- 支持伙伴筛选；大字号时筛选改为 Menu，不保留挤压的横向 chips。

### 6.6 创作首页

只展示 V1 可用项目：拼豆图纸、伙伴卡片。每项使用“原图 → 成品”的示例视觉，而非图标功能列表。图片编辑器从照片详情进入。

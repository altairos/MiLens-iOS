...
/// - Parameter includeWatermark: true 时页脚显示醒目 MiLens 水印（免费版），false 时只保留日期（Pro）。
public func renderA4Export(
    ...
    // ADR-0010：免费版显示醒目水印（页脚区域），Pro 版只保留日期。
    // 位图字体仅支持 ASCII，水印文案用英文。
    let footerText = includeWatermark
        ? "Made with MiLens  |  milens.app  |  \(dateStr)"
        : "Generated by MiLens  |  \(dateStr)"
    let footerScale = includeWatermark ? 3 : 2
    let footerColor: (UInt8, UInt8, UInt8) = includeWatermark ? (100, 100, 100) : (160, 160, 160)
    drawString(&pixels, bufW: A4_W, bufH: A4_H, text: footerText,
               x0: MARGIN, y0: A4_H - MARGIN - FOOTER_H + 20, scale: footerScale,
               r: footerColor.0, g: footerColor.1, b: footerColor.2)
